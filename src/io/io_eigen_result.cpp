...
int IoEigenResult::readEigenFile(std::string fileName) {
...
int IoEigenResult::readEigenvalue(std::string& eigenvalueStr) {
    // 指定所需空间
    eigenvalue_.reserve(numEigenvalue_); 
...
int IoEigenResult::readEigenVector(std::string& eigenVectorStr) {
    std::vector<double> vectorTemp; // 临时存放特征向量 todo 应该可用共用一个
